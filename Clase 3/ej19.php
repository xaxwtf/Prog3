...
class Auto {
    ...
    public function AgregarImpuestos($imp1=0){
        $this->_precio=$this->_precio+$imp1;
    }
    ...
    public static function VolcarAutosEnArchivo($lista,$name){
        $archivo=fopen($name,"w");
        foreach($lista as $auto){
            echo fwrite($archivo, json_encode($auto));
        }
        fclose($archivo);
    }
    public static function LeerAutosDeArchivo($name){
        $r;
        $archivo = fopen($name,"r");
        $r = fread($archivo, filesize($name));
        if($r!=false){
            $r=json_decode($r);
        }
        return $r;
